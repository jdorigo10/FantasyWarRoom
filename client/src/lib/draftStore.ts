...
export const useDraftStore = create<DraftState>(
    (set, get) => ({
      ...
      rankingsFilters:
          {search: '', pos: 'All', team: 'All', showDrafted: false},
      playerTags: {},
        ...
        let candidates =
            availablePlayers
                .filter(p => {
                  ...
                  // Round restrictions for QB (don't take 2nd QB before round
                  // 10)
                  if (pos === 'QB' && round < 10 && counts.QB >= 1)
                    return false;

                  // Round restrictions for TE (don't take 2nd TE before round
                  // 8)
