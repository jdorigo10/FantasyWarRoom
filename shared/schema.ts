...
export const draftSessions = pgTable('draft_sessions', {
  ...
  settings:
      jsonb('settings').notNull(),  // Draft settings (teamCount, scoring, etc.)
  picks: jsonb('picks').notNull().default(sql`'[]'::jsonb`),  // Array of picks
  currentPickIndex: integer('current_pick_index').notNull().default(0),
  playerTags: jsonb('player_tags')
                  .notNull()
                  .default(sql`'{}'::jsonb`),  // Player favorite/target tags
...
export const userPreferences = pgTable('user_preferences', {
  ...
  accentColor: text('accent_color').notNull().default('#388bfd'),
...
export const insertUserPreferencesSchema =
    createInsertSchema(userPreferences).omit({
      id: true,
      createdAt: true,
      updatedAt: true,
    });
